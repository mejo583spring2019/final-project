...
class BlackChart extends BarChart {
  constructor(props) {
    ...
    this.fullData.forEach((s) => {
      if (s !== undefined && s["latest.completion.completion_rate_4yr_150_black"] !== null) {
        schoolName.push(s["school.name"]);
        blackCompletion.push(s["latest.completion.completion_rate_4yr_150_black"]);
      }
    });
    console.log(blackCompletion);
